...
import { api, createInitialState, setLoading, setData, setError } from '../utils/api';
// Hook for overview analytics
export function useOverviewAnalytics(surveyId) {
...
export function useAIReadinessAnalytics(surveyId) {
    const [state, setState] = useState(createInitialState());
    const fetchData = useCallback(async () => {
        setState(prev => setLoading(prev));
        try {
...
export function useLeadershipAnalytics(surveyId) {
    const [state, setState] = useState(createInitialState());
    const fetchData = useCallback(async () => {
        setState(prev => setLoading(prev));
        try {
...
export function useEmployeeExperienceAnalytics(surveyId) {
    const [state, setState] = useState(createInitialState());
    const fetchData = useCallback(async () => {
        setState(prev => setLoading(prev));
        try {
...
export function useAllModuleAnalytics(surveyId) {
