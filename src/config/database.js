...
export const API_ENDPOINTS = {
    ...
    responses: {
        ...
        getBySurvey: (surveyId) => `${API_BASE_URL}/responses/survey/${surveyId}`,
        getByUser: (userId) => `${API_BASE_URL}/responses/user/${userId}`,
    },
    // Analytics
    analytics: {
        overview: (surveyId) => `${API_BASE_URL}/analytics/overview${surveyId ? `?surveyId=${surveyId}` : ''}`,
        aiReadiness: (surveyId) => `${API_BASE_URL}/analytics/ai-readiness${surveyId ? `?surveyId=${surveyId}` : ''}`,
        leadership: (surveyId) => `${API_BASE_URL}/analytics/leadership${surveyId ? `?surveyId=${surveyId}` : ''}`,
        employeeExperience: (surveyId) => `${API_BASE_URL}/analytics/employee-experience${surveyId ? `?surveyId=${surveyId}` : ''}`,
    },
    // Real-time data
    realtime: {
